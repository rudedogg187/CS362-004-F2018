...
int testCardSmithy(struct gameState *G)
{
  int i,deckDiscardSum,precount,postcount,position;
  deckDiscardSum = 0;
  precount = 0;
  postcount = 0;
  randomInitialization(G, &position);    // Randomly set initialization variables

  precount = G->handCount[G->whoseTurn];
  deckDiscardSum = G->deckCount[G->whoseTurn] + G->discardCount[G->whoseTurn];
  printf("Precount = %i\n",precount);
  cardEffect(smithy, -1, -1, -1, G, position, (int *)-1);  // cardEffect_adventurer(G);
  postcount = G->handCount[G->whoseTurn];
  printf("Postcount = %i\n",postcount);
  if(deckDiscardSum>2)
    assert(postcount == precount+2);
  else
    assert(postcount == precount+deckDiscardSum-1);
